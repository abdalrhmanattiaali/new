...
أنت مساعد عائلي ذكي وحَنون، دورك دعم الوالدين بوضوح واحترام.
- استخدم لغة عربية بسيطة، مختصرة، ودافئة دون إفراط، مع نبرة مهنية مشجعة.
- قدّم خطوات عملية محددة وأمثلة واقعية تناسب العمر الظاهر، واذكر يوم الطفل في حياته متى وُجد.
- اعرض التفهّم والتقدير لمجهود الوالدين (مثل: «أقدّر شعورك»، «هذا طبيعي أن يحدث»، «يمكننا معًا تجربة ما يلي…») وتجنب اللوم.
- اربط كل رسالة بالسجل السابق وعدد الإشعارات/التسلسل لتجنب التكرار وإبراز القيمة المضافة.
- عند ظهور صحة جسدية/نفسية مقلقة أو سلوك خطر، نبّه بلطف وشجع على استشارة طبيب أطفال أو مختص، وامتنع عن أي نصيحة قد تُعرّض الطفل للخطر.
- احترم القيم العائلية والدينية بلطف، وركز على مصلحة الطفل وسلامته.
    `.trim();
  ...
  async generateInferenceQuestion(context) {
    const prompt = this.buildInferenceQuestionPrompt(context);
    const raw = await this.runModel(prompt, 900);
    const parsed = this.extractJSON(raw);
    if (!parsed?.question) {
      throw new Error('Inference question response missing question text');
    }
    return parsed;
  }
...
- كيفية تطبيقه هذا الأسبوع (خطوة واحدة قابلة للتنفيذ).
      ...
      parent_video: `
اقترح 2-3 فيديوهات قصيرة من YouTube أو Facebook (روابط مباشرة من النطاقات المسموح بها فقط).
${additionalContext ? `المجالات المسموح بها: ${additionalContext}\n` : ''}

التوزيع:
- خيار مهدئ أو ترفيهي للطفل مع مدة تقديرية.
- خيار استرخاء سريع للأم أو الأب (تنفس، يوغا قصيرة، ضحك).
- خيار عائلي لطيف يمكن مشاهدته معاً مساء اليوم.

لكل فيديو اذكر:
- عنوان مختصر + المنصة.
- سبب الترشيح ولماذا يناسب حالة العائلة الحالية.
- مدة المشاهدة التقريبية.
- رابط مباشر واحد فقط من النطاقات الموثوقة.
      `.trim(),

      parent_education_hub: `
أنت مساعد تعلّم للأسرة. أنشئ حزمة موارد ذكية للوالدين ${guardianName || 'العائلة'} تساعدهما اليوم في دعم ${childName} (اليوم رقم ${childDay || '؟'} من عمره).
${additionalContext ? `تفاصيل إضافية: ${additionalContext}\n` : ''}

المطلوب:
- مقطع قصير لطبيب أطفال أو خبير موثوق مع رابط مباشر من النطاقات المسموحة.
- اقتراح كتاب عملي أو ملخص كتاب مع خطوة تطبيقية خلال 48 ساعة.
- توصية كورس مصغر (أقل من 60 دقيقة) أو تمرين ذهني/عقلي يمكن للأبوين تجربته الليلة.
- فيديو استرخاء/تنفس سريع للأب أو الأم يعكس حالتهم الحالية.

التزامات:
- استخدم لغة عربية مهنية دافئة، مع إبراز اليوم الفعلي لعمر الطفل.
- لا تكرر عناوين أو روابط ذُكرت سابقاً في السجل أدناه.
- قدّم كل مورد في فقرة منفصلة بخطوات واضحة، واذكر المدة التقريبية.
- اربط نصيحة الأب/الأم بكون الأب داخل/خارج البيت إن وُجدت معلومة، وأضف جملة تقدير لمجهودهما.
      `.trim(),
...
أصدر قراراً واحداً بصيغة JSON:
{
  ...
  buildInferenceQuestionPrompt(context) {
    const {
      familyName,
      guardians = [],
      children = [],
    ...
    const childText = children
      .map((child) => `${child.name} - يومه ${child.day_of_life || '?'} (${child.development_stage || 'غير محدد'})`)
      .join(', ');
    const digestText = notificationDigest
      .map((row) => `#${row.sequence} ${row.message_type} @ ${row.ts || 'N/A'} => ${row.snippet}`)
      .join('\n');
    const coupleText = coupleInsights
      .map((note) => `${note.sentiment}: +${note.positives_text || ''} / -${note.challenges_text || ''}`)
      .join(' || ');
    const issuesText = activeIssues.map((issue) => `${issue.issue_title || issue.issue_type} (${issue.status})`).join(' | ');

    return `أنت مساعد عائلي ذكي تريد طرح سؤال استنباطي قصير يساعدك على فهم الحالة الحالية للأسرة وتحديث الترجيحات.
العائلة: ${familyName}
الأوصياء: ${guardianText || 'غير مسجل'}
الأطفال: ${childText || 'غير مسجل'}
المشاكل النشطة: ${issuesText || 'لا يوجد'}
ملاحظات العلاقة: ${coupleText || 'لا يوجد'}
أحدث الإشعارات:
${digestText || 'لا يوجد'}
الموضوع المفضل إن وجد: ${seedTopic || 'عام'}

أعد مخرجات JSON فقط:
{
  "question": "سؤال استنباطي واحد بالعربية الفصحى الدافئة",
  "follow_up": "سؤال متابعة صغير إن لزم الأمر أو null",
  "topic": "كلمة مفتاحية للغرض (طفل، علاقة، تنظيم يوم)",
  "target": "father" أو "mother" أو "both",
  "priority": "low"|"medium"|"high",
  "action_hint": "كيف سنستخدم الإجابة (تعديل الجداول، ترجيح التنبيهات...)"
}`;
  }
  ...
  generateFallbackMessage(messageType) {
    const fallbacks = {
      ...
      parent_video: '🎥 فيديو قصير اليوم: استمتعوا بمشاهدة رابط موثوق من YouTube أو Facebook للاسترخاء مع العائلة.',
      parent_education_hub: '🎓 حزمة التعلم الذكية: كتاب عملي + كورس مصغر + فيديو موثوق من YouTube/Facebook + تمرين استرخاء قصير للوالدين.',
