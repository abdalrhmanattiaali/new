...
/**
 * نموذج الأسئلة الاستنباطية
 */
export class InferenceQuestionModel {
  static create({ familyId, guardianId = null, topic = null, questionText, followUp = null, aiPayload = null, metadata = null }) {
    const db = getDatabase();
    const stmt = db.prepare(`
      INSERT INTO inference_questions (
        family_id, guardian_id, topic, question_text, follow_up, ai_payload, status, metadata
      )
      VALUES (?, ?, ?, ?, ?, ?, 'pending', ?)
    `);
  ...
  static markAnswered(familyId, answerText = null) {
    const db = getDatabase();
    const row = db
      .prepare(
        `SELECT id FROM inference_questions
    ...
    db.prepare(
      `UPDATE inference_questions
       SET status = 'answered', answered_at = CURRENT_TIMESTAMP, answer_text = ?, updated_at = CURRENT_TIMESTAMP
       WHERE id = ?`
    ).run(answerText || null, row.id);
  }

  static hasRecentPending(familyId, hours = 12) {
    const db = getDatabase();
    const row = db
      .prepare(
        `SELECT 1 FROM inference_questions
         WHERE family_id = ?
           AND status = 'pending'
           AND asked_at >= datetime('now', ?)
         ORDER BY asked_at DESC LIMIT 1`
      )
      .get(familyId, `-${hours} hours`);

    return !!row;
  }

  static getRecentByFamily(familyId, limit = 10) {
    const db = getDatabase();
    return db
      .prepare(
        `SELECT * FROM inference_questions
         WHERE family_id = ?
