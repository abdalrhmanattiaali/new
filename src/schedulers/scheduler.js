...
export class Scheduler {
  ...
  initialize() {
    ...
    const inferenceConfig = this.config.inference_questions || {};
    if (inferenceConfig.enabled !== false) {
      const cronTime = inferenceConfig.cron_time || '08:40';
      const [iqHour, iqMinute] = cronTime.includes(':') ? cronTime.split(':') : cronTime.split(' ');
      this.scheduleJob(
        `${iqMinute} ${iqHour} * * *`,
        'Inference Probing Questions',
        () => this.inferenceQuestions.runScheduledQuestions()
      );
    }
